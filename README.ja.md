...
macOS のバックグラウンドプロセス(LaunchAgent)は「ローカルネットワーク」プライバシーで
カメラ(172.x)への接続を無音で拒否される。許可プロンプトも出ない。設定でトグルを ON に
しても launchd 経由には効かない。**裸の実行ファイルには、macOS が許可を紐づける
「アプリの身元」が無いため。**

そこで gpget は 2 段構えになっている:

1. **LaunchAgent**(5 秒間隔)は**ネットワークに触れない。**
   「GoPro らしい USB 有線 IF が生えたか」だけを見る(1 回 10ms 未満)。
   口がまだ無いときは `ioreg` で GoPro の USB を確認し、居れば最大 30 秒待つ
2. 検出したら `~/Library/Application Support/gpget/gpget.app` を起動する。
   これが実際にカメラと話す。**身元があるので通信が許可される**

この `.app` は `LSUIElement` 指定なので、**ウィンドウも Dock アイコンもメニューバー
項目も出ない。**中身は gpget のバイナリそのもので、`gpget autostart install` が
組み立てて ad-hoc 署名する。配布物は単一バイナリのまま。

初回だけ「通知を許可しますか」と「ローカルネットワークを許可しますか」が出るので、
どちらも「許可」を押す。以後は自動。`gpget autostart install` の最後にテスト通知が
飛ぶので、**そこでバナーが見えるかを必ず確認すること**(見えないと結果が分からない)。
見えなければ システム設定 > 通知 > gpget を ON にする。
...
- アプリ実体:`~/Library/Application Support/gpget/gpget.app`(再インストールで作り直し)
